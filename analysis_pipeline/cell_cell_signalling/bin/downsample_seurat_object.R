...
filename <- gsub(file_sc_obj, pattern = ".rds", replacement = paste0("_downsampled", downsample, ".rds"))

# If no downsampling desired, just copy the file to save time reading and writing
if (downsample == 1) {
    file.copy(from = file_sc_obj, to = filename)
    quit("no")
}
